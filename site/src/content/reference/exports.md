...
注册自定义结构，提供结构类型与实现组件后即可在模板中使用。结构类型见 [Structure](/reference/types#structure)。
...
注册自定义数据项组件，传入类型与组件定义。数据项类型见 [BaseItemProps](/reference/types#base-item-props) 与 [Item](/reference/types#item)。
...
拆分数据项的容器参数与组件参数，便于在结构组件中统一处理。输入输出基于 [BaseItemProps](/reference/types#base-item-props)。
...
注册自定义字体，返回记录了编码后 `fontFamily` 的字体对象。字体结构见 [Font](/reference/types#font)。
...
> 部署字体资源详见：https://chinese-font.netlify.app/zh-cn/post/deploy_to_cdn
...
注册一组色板，可为数组或返回色值的函数。色板类型见 [Palette](/reference/types#palette)。
...
注册主题配置，主题名可用于后续获取。主题结构见 [ThemeConfig](/reference/types#theme-config)。
...
根据主色、背景色和信息图配置生成一组主题衍生色，返回值见 [ThemeColors](/reference/types#theme-colors)。
...
注册自定义资源加载器，用于处理 `type: 'custom'` 的资源。签名见 [ResourceLoader](/reference/types#resource-loader)。
...
注册模板配置，使用模板类型标识。模板结构见 [TemplateOptions](/reference/types#template-options)。
